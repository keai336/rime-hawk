--[[
    aux_refactored_v2.lua
    版本: v2 (release)

    基于 aux_refactored_v1 (核心重构: Memory API 替代音码列)

    修复:
    - 修音分支中 inputspls 被就地修改，污染 split_pinyin 缓存导致崩溃
      → 改为从原始 preedit 创建新数组，避免污染缓存
    - 增加越界安全检查，防止崩溃
    - handle_no_match 兜底候选由零长构造改为正确的 _start/_end 范围
    - 主动分心分支补齐首字锁定校验，与被动分心保持一致
...
local function get_syllable_aux_set(syllable)
    ...
    local function collect_aux_from_iter()
        local raw_aux_set = {}
        ...
        return raw_aux_set, has_any, single_char_count
    end
    ...
    local raw_aux_set, has_any, single_char_count

    if lookup_result then
        raw_aux_set, has_any, single_char_count = collect_aux_from_iter()
